...
totalGoals = result[0].totalGoals
}


export const loadCRUD = async (req, res) => {
  try {
    await findTotalGoals();
    ...
    if (sortBy) {
      const playerNameToSearch = req.query.sortBy;
      players = await Player.find({ playerName: playerNameToSearch });
    } else {
      players = await Player.find();
    }

    res.render('crud', { title: "CRUD", players, sortingByName, sortingByNumber, sortingByGoals, totalGoals });
  } catch (error) {
